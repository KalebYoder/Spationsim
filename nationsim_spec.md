...
- [ ] Consolidation window — new `consolidating` fleet status set when last defender fleet is destroyed and territory becomes uncontested; `consolidation_expires_at` stored on fleet row; tick promotes expired consolidating fleets to withdrawn (fleet recalled); window cancelled if enemy arrives; `POST /fleets/{id}/consolidate` endpoint triggers instant conquest + dissent-set-to-60 + upkeep start
...
| Consolidation window | 6-tick (12-hour) forced-choice after last defender fleet destroyed and territory uncontested; default = withdraw | Attacker chooses consolidate (instant conquest, upkeep begins, dissent set to 60) or withdraw (fleet recalled). Window cancelled if enemy units arrive during the window. Default on expiry = withdraw (inaction = safe). Early claim or early withdraw both allowed. Dissent set to 60 fires on consolidate action only — not when defenders are destroyed — to prevent repeated conquer-and-withdraw as a dissent weapon. Fleet row stores `consolidation_expires_at`. Endpoints: `POST /fleets/{id}/consolidate` and `POST /fleets/{id}/withdraw` (withdraw already exists; consolidate is new). |
