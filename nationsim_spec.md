...
| Occupation window | 6-tick (12-hour) forced-choice after last defender fleet destroyed and territory uncontested; default = withdraw | Attacker fleet enters `occupying` status. Attacker chooses occupy (instant conquest, upkeep begins, dissent set to 80) or withdraw (fleet recalled). Window cancelled if enemy units arrive during the window — fleet reverts to `holding`. Default on expiry = withdraw (inaction = safe). Early occupy or early withdraw both allowed. Dissent set to 80 fires on occupy action only — not when defenders are destroyed — to prevent repeated defeat-and-withdraw as a dissent weapon. Fleet row stores `occupation_expires_at`. Endpoints: `POST /fleets/{id}/occupy` and `POST /fleets/{id}/withdraw` (withdraw already exists; occupy is new). |
| Post-conquest costs | `DISSENT_CONQUEST_RESET = 80`; 12-tick (24h) construction lockout; Propaganda Office destroyed on conquest, 20 pop permanently lost | Three-part fix for the "conquest has very low lasting cost" problem. Lockout stored as `conquest_lockout_until` TIMESTAMPTZ on the territory row; `build_facility` rejects all builds until it expires. PO destruction fires a `propaganda_office_destroyed` event; the 20 assigned population is not restored to the territory's free pool. Combined timeline: at -3/tick (no PO), dissent falls from 80 to 44 during the 12-tick lockout — still above the production penalty threshold (25). The conqueror can rebuild a PO after the lockout; full recovery completes ~9 ticks later (~21 ticks / 42 hours total post-conquest). |
...
| Post-battle choice window | 2 ticks (4 hours), matching the fleet arrival confirmation window | Same reasoning as the confirmation window: 2 hours is insufficient for an async player. A player offline for 8 hours during a multi-tick war gets 2 choices expiring (not 4). Implemented via `_CONFIRMATION_WINDOW` constant in `tick.py`. |
...
| Raid formula | `loot = min(fleet_FP × RAID_K, defender_stockpile × RAID_MAX_STOCKPILE_FRACTION)` per resource; `RAID_K = 1`, `RAID_MAX_STOCKPILE_FRACTION = 0.05` | Replaces the production-ticks-cap model, which made fleet size irrelevant (cap always bound above a trivial fleet size). New formula scales linearly with combined fleet FP until hitting 5% of the defender's national stockpile. 500 FP (e.g. 250 starfighters, or a smaller mixed fleet with cruisers) hits the cap against a 10,000-unit stockpile. Genre comparators (OGame, P&W) use per-planet or per-battle stockpile fractions; our centralized stockpile model makes per-planet storage unnecessary but requires a small fraction cap to prevent a single raid from draining a large empire. Random draw removed as it was irrelevant — the cap always bound above trivial fleet sizes anyway. |
...
**~~*(2026-06-13)* Tutorial rewards are not reverted on construction cancellation — can be exploited for free resources.~~** *(Resolved — tutorial steps 1 and 2 (mine and refinery) now instantly activate the facility (`status = "active"`, `completes_at = None`) when the tutorial reward fires. Active facilities cannot be cancelled — only demolished at 25% refund — so the exploit is closed. Tutorial panel text updated to note that normal facilities take 1 tick to build and these first two are exceptions.)*

**~~*(2026-06-13)* `sortie_queued` flag on a territory is never cleared when the attacker manually acts before the `post_battle_choice` window expires.~~** *(Resolved — the actual paths out of `post_battle_choice` before expiry are `rout` and `raid` (both set fleet to `engaged`). Both endpoints now clear `dest.sortie_queued = False` before committing. The sortie intent is fulfilled when the attacker engages, so consuming the flag is correct. The tick's PBC-expiry block already cleared it on the expiry path — this closes the proactive-action path.)*

**~~*(2026-06-13)* The `rout` endpoint calculates bonus damage by querying the most recent `combat_round` event for the fleet, with no filter for the current war or current engagement.~~** *(Resolved — `rout_fleet` now fetches the diplomacy row for the nation pair and filters the `combat_round` query by `Event.scheduled_for >= diplo.war_starts_at`, excluding events from prior wars at the same territory. No schema change required.)*

**~~*(2026-06-13)* Colony ship population check uses `Infrastructure.population_assigned` column, which is never written by any endpoint — it defaults to 0 for all facilities.~~** *(Resolved — `load_colony_ship` now computes `assigned_pop` using `sum(FACILITY_POPULATION_COST.get(f.type, 0) for f in territory_facilities)` filtering on `active` and `under_construction` facilities, consistent with every other population-reservation check in the codebase.)*

**~~*(2026-06-13)* The `auto_rout` block in `tick.py` fires and logs an event even when the attacker fleet was zeroed by the main combat round in the same tick.~~** *(Resolved — guard changed to `if attacker_losses > 0 and fleet.unit_count > 0 and defender_fleet.unit_count > 0`. When the attacker fleet is already at zero from main combat, the auto-rout block is skipped entirely.)*

**~~*(2026-06-13)* War declaration is not protected against a race condition between the two declaration paths (`POST /api/diplomacy/war` and `PUT /api/diplomacy/{target_nation_id}`).~~** *(Resolved — `_get_or_create_diplomacy` now uses `.with_for_update()` on the SELECT, issuing a `SELECT FOR UPDATE`. Concurrent declarations on an existing diplomacy row will serialize: the second transaction waits for the first to commit, then sees `war_pending` and hits the idempotency return. Covers the common case; the brand-new-row INSERT race is theoretical at beta scale.)*
...
**~~War declaration allows fleets already in transit to proceed unimpeded through the 2-tick grace period.~~** *(Resolved — two-part fix in `tick.py`: (1) fleets arriving at an enemy planet during `war_pending` land as `holding` rather than `stationed` (fires `enemy_fleet_holding_at_border` event; no attrition until war activates); (2) when a `war_pending` row promotes to `war`, a sweep converts any `stationed` or `holding` fleet at enemy planets to `pending_confirmation` with the standard 4-hour window, firing `fleet_arrived_at_enemy_territory` and `enemy_fleet_arrived` events. Net effect: short-range attacks (1 node) have a 6-hour minimum before combat; 2+ nodes retain the 8-hour minimum. Covered by `tests/test_war_pending_transit.py`.)* When a war is declared, a `war_pending` row is created and hostilities begin 4 hours later. However, fleets dispatched before declaration (in status `in_transit`) continue to travel during the grace period and will arrive at the enemy territory the moment `war` becomes active, potentially before the defender has received a tick notification. The spec says the grace period exists so defenders can "see the war declared notification and prepare" — a fleet that departs the same tick war is declared can arrive the same tick war becomes active, negating the grace period entirely for short distances.
...
**~~*(2026-06-13)* No automatic war-end trigger when one side is eliminated or goes inactive.~~** *(Resolved — tick.py auto-resolves any `war` row where `war_started_at <= now − WAR_MAX_DURATION_DAYS (14)` to `neutral`. Fires a `war_auto_ended` event for both parties. Sets `peace_until = now + 48h` to prevent immediate re-declaration. `WAR_MAX_DURATION_DAYS` in `constants.py` is the tuning lever. Territory-elimination auto-surrender deferred to post-beta.)*

**~~*(2026-06-13)* Non-owner fleet transit through friendly or neutral territory fires no notification to the territory owner.~~** *(Resolved — `send_fleet` now calls `compute_path()` (new function in `services/pathfinding.py`) to find the BFS path from origin to destination, then fires a `foreign_fleet_in_transit` event for each distinct territory owner whose space is crossed. Deduplicated per dispatch — a player with multiple consecutive intermediate territories gets one notification, not one per tile. Fires at dispatch time, not each tick.)*

**~~*(2026-06-13)* There is no cap on concurrent `under_construction` facilities per territory, allowing population to be double-committed across simultaneously queued builds.~~** *(Resolved — `territory_population.current` now stores free (unassigned) population only. `build_facility` deducts `FACILITY_POPULATION_COST[type]` from `current` immediately (with `SELECT FOR UPDATE`), so subsequent builds at the same territory see the already-reduced value. Cancel and demolish-complete restore the pop. The double-commit race is structurally impossible: each check sees the live free-pop total.)*

#### Resolved

**~~*(2026-06-05)* No notification on fleet arrival at an enemy territory.~~** *(Resolved — three-layer plan fully executed. (1) In-game alert badge: red badge on Military sidebar nav, polled every 45s via `GET /api/notifications` `threat_count`; `GET /api/military/fleets/pending-at-mine` supplies the defender view. (2) Browser notification (Notification API): `fleet_pending_action` event fires a native `Notification` when pending fleet counts increase; bell icon in sidebar requests permission. Fires while page is open. Full Web Push API intentionally out of scope. (3) Email: infrastructure implemented (migration 018, `send_email` service, `send_email_task` Celery task, `email_notifications_enabled` opt-in); fires for account events (welcome) and attacker post-mortem when confirmation window expires unresolved. Fleet arrival itself intentionally excluded from email — "do not email on every fleet arrival during war.")*
...
**~~*(2026-06-06)* The `is_colonized` flag on `Territory` is semantically overloaded, causing a confusing UX failure for colony ship dispatch.~~** *(See Best Practice Violations — Resolved.)*

**~~*(2026-06-06)* Email notifications are not yet implemented.~~** *(Resolved — `send_email` service + `send_email_task` Celery task implemented. `email_notifications_enabled` field on `Player` model (migration 018 applied). Welcome email fires on registration; expired-confirmation-window email fires when a `pending_confirmation` fleet lapses. Toggle via `PUT /api/auth/notifications`. No-ops silently when `SMTP_HOST` is blank — fully dormant in dev/beta until SMTP credentials are added to `.env`. Web Push API remains intentionally out of scope.)*
---
...
**~~*(2026-06-13)* Raid's random draw is irrelevant above a small fleet size — the production cap always binds, making raid output flat regardless of fleet size.~~** *(Resolved — raid formula replaced entirely. New formula: `loot = min(fleet_FP × RAID_K, defender_stockpile × RAID_MAX_STOCKPILE_FRACTION)` per resource. `RAID_K = 1`, `RAID_MAX_STOCKPILE_FRACTION = 0.05`. 500 FP hits the 5% stockpile cap on a 10,000-unit stockpile. Scales naturally with future unit types (cruisers, etc.) via their FP stat. Random draw and production-ticks-cap both removed. See Decisions Log.)*

***(2026-06-13)* `HOME_TERRITORY_DEFENSE_MULTIPLIER` effectively requires closer to 2:1 attacker superiority for casualty parity, not the 1.5:1 described in the spec.** The multiplier applies only to how many defenders the attacker kills (defender effective = `defender_count × 1.5`), but the defender fires against the real attacker count. At 150 attackers vs 100 defenders: attacker kills 30 defenders, defender kills 10 attackers, auto-rout adds 5 more — exchange rate is 30:15 (2:1). The spec's documentation says "attacker needs ~1.5× numerical superiority to achieve casualty parity" — the actual crossover is approximately 2:1. The mechanic may be working as intended, but the spec characterization is inaccurate. Update either the constant (lower to ~1.25×) or the spec language to accurately reflect the real exchange rate, so beta players have correct expectations.

**~~*(2026-06-13)* Post-conquest dissent decays to below the production threshold within ~14 hours with a Propaganda Office, making territorial conquest have very low lasting cost for the conqueror.~~** *(Resolved — three-part fix applied: (1) `DISSENT_CONQUEST_RESET` raised from 60 → 80; (2) 12-tick (24h) construction lockout stored as `conquest_lockout_until` on the territory row — `build_facility` rejects all builds until it expires; (3) any Propaganda Office on a conquered territory is destroyed and its assigned population (20 pop) is permanently lost — it cannot be transferred to the new government. Combined effect: without a PO, dissent decays at -3/tick from 80; after 12 ticks (lockout expires, PO can be rebuilt) dissent is at 44 — still above the production threshold (25). Full recovery only completes ~9 ticks after the PO is rebuilt, ~21 ticks (42h) post-conquest total. See Decisions Log.)*

#### Resolved

**~~*(2026-06-06)* The `post_battle_choice` window is 1 tick (2 hours), inconsistent with the 4-hour confirmation window and punishing for asynchronous play.~~** *(Resolved — `tick.py` now uses `_CONFIRMATION_WINDOW` (4 hours / 2 ticks) for the post-battle choice window, matching the fleet arrival confirmation window. A player fighting a multi-tick war who is offline for an 8-hour stretch gets 2 choices expiring rather than 4. Inaction on expiry = holding (safe default). See Decisions Log.)*
