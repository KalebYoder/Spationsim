...
***(2026-06-06)* Browser push notifications and email notifications are not yet implemented.** The three-layer fleet-arrival alert plan (in-app badge, browser push, email) has only layer 1 complete. Layer 2 — Web Push API notifications fired by Celery on window open and at T-1 tick — and layer 3 — opt-in email for account events and expired windows — are both unimplemented. For a 20–50 player closed beta where players are expected to respond to fleet arrivals within 4 hours, the in-app badge is sufficient only if players happen to check. Veterans accustomed to P&W or CyberNations will expect email or push; the lack of out-of-session alerts will be the most-cited friction point in early beta feedback. Layer 2 (Web Push) should be prioritized before public invite.

***(2026-06-06)* The `is_colonized` flag on `Territory` is semantically overloaded, causing a confusing UX failure for colony ship dispatch.** A fleet claiming an unclaimed territory sets `is_colonized = True` and `nation_id = <player>`. The territory is now owned. However, `send_colony_ship` at `military.py` line 979 gates dispatch on `dest.is_colonized == True and dest.nation_id == nation.id`, which would pass — but wait: `claim_territory` at line 789 sets `is_colonized = True`. So the colony ship CAN be sent there. But the endpoint description says colony ships can only travel to "your own colonized territories" — and in practice, a fleet-claimed territory has no population row, so unloading there will work but the territory will have 0 pop until unload. The more precise issue: the UI and docs call this territory "claimed" (by fleet) vs "colonized" (populated), but `is_colonized` is `True` in both cases. Players reading the Planets page or tutorial prompts that use "colonized" may not understand they still need to unload a colony ship to staff facilities. This is a documentation and UI labeling issue more than a code bug, but it will cause repeated confusion in beta. Consider renaming the flag to `is_owned` or surfacing a separate `is_populated` state in the UI.
