...
***(2026-06-05)* No notification on fleet arrival at an enemy territory.** The confirmation window protection only works if players see the alert. Notification design (do not email on every fleet arrival during war — veteran players will unsubscribe): (1) **In-game alert badge** on the sidebar — increments for every `pending_confirmation` fleet at the player's territories, shows territory, fleet power, and expiry countdown. Prerequisite: `GET /api/military/fleets/pending-at-mine`. (2) **Browser push notifications** (Web Push API, no mobile app required) — Celery fires on window open and again at T-1 tick (2 hours remaining). (3) **Email** only for account events and post-mortem when a window expired unresolved; user opt-in, default off.
